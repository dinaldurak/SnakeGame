...
public class Snake {
    ...
    private ArrayList<SnakeParts> arrPartSnake = new ArrayList<>();
    private int length;
    ...
    public Snake(Bitmap bm, int x, int y, int length) {
        ...
        setMove_right(true);
        arrPartSnake.add(new SnakeParts(bm_head_right, x, y));
        for (int i = 1; i < length-1; i++){
            this.arrPartSnake.add(new SnakeParts(bm_body_horizontal, this.arrPartSnake.get(i-1).getX()-GameView.mapSize, y));
        }
        arrPartSnake.add(new SnakeParts(bm_tail_right, arrPartSnake.get(length-2).getX()-GameView.mapSize, arrPartSnake.get(length-2).getY()));
    }
    ...
    public void update(){
        ...
        if(move_right){
            arrPartSnake.get(0).setX(arrPartSnake.get(0).getX()+GameView.mapSize);
            arrPartSnake.get(0).setBm(bm_head_right);
        }else if(move_down){
            arrPartSnake.get(0).setY(arrPartSnake.get(0).getY()+GameView.mapSize);
    ...
    public ArrayList<SnakeParts> getArrPartSnake() {
        return arrPartSnake;
    }

    public void setArrPartSnake(ArrayList<SnakeParts> arrPartSnake) {
    ...
    public void addPart() {
        SnakeParts p = this.arrPartSnake.get(length-1);
        this.length += 1;
        if(p.getBm()==bm_tail_right){
            this.arrPartSnake.add(new SnakeParts(bm_tail_right, p.getX()-GameView.mapSize, p.getY()));
        }else if(p.getBm()==bm_tail_left){
            this.arrPartSnake.add(new SnakeParts(bm_tail_left, p.getX()+GameView.mapSize, p.getY()));
        }else if(p.getBm()==bm_tail_up){
            this.arrPartSnake.add(new SnakeParts(bm_tail_up, p.getX(), p.getY()+GameView.mapSize));
